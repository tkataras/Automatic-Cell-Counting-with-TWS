...
inputdir_points <- "F:/Theo/iba_7_2020_autocount/working images/substack32_37/substack sub max project 32_37 roi blobs too/" #all handcounts currently, 20
inputdir_points <- dir(inputdir_points)

inputdir_wtgp_prev <- "F:/Theo/iba_7_2020_autocount/working images/substack32_37/wt and gp images that have been counted previously/"
inputdir_wtgp_prev <- dir(inputdir_wtgp_prev)

inputdir_wtgp_tot <- "F:/Theo/iba_7_2020_autocount/working images/substack32_37/all wt and gp images in dataset/"
inputdir_wtgp_tot <- dir(inputdir_wtgp_tot)

inputdir_training_and_val <- "F:/Theo/iba_7_2020_autocount/working images/substack32_37/quarters_for_training_and_validation/"
inputdir_training_and_val <- dir(inputdir_training_and_val)

inputdir_new_male <-"F:/Theo/iba_7_2020_autocount/working images/substack32_37/new training/random_quarters/new_male_only_6_24/roi/" #new onlymale training roi
inputdir_new_male <- dir(inputdir_new_male)

#id1 will = used images
#id1 <- c(inputdir_points,inputdir_training_and_val)
id1 <- inputdir_all
#id2 will = all previously counted wt and gp images, to finish out the male
id2 <- inputdir_new_male


outdir <- "F:/Theo/iba_7_2020_autocount/working images/substack32_37/new training/random_quarters/new_male_only_6_24/" #all images of counts currenlty
...
# ###MAKINg THIS MALE ONLY***********************************************************
# m = c(426,428,443,442,460,462)
# id2_df <- id2_df[newsid2_anum %in% m, ]
...
duplicated(id1_df_squish)### using unique and duplicate commands probably better than the following loops
duplicated(c(id1_df_squish, id2_df_squish))
sum(duplicated(c(id1_df_squish, id2_df_squish)))

unique_id1 <-unique(id1_df_squish) ##unique = everything once, even if it has a duplicate

dup_id1 <- id1_df_squish[duplicated(id1_df_squish)]
duplicated(dup_id1)#checks for duplicates of more than 1x(eg triplicate and more)
...
length(id2_df_squish)
length(unique(id2_df_squish))
id2_df_squish[duplicated(id2_df_squish)]


##looking for object of df1 IN df2, coming out with row numbers in df2
match_id <- NA
for (i in 1:length(unique_id1)){
  pineapple <- grep(unique_id1[i],id2_df_squish)
...
matches_in2_withu1 <- id2_df_squish[match_id]
imgs_in2_NOT_1 <-id2_df_squish[-match_id]
length(imgs_in2_NOT_1)
...
#grabbing squished names, will hvae to move manually

dim(id2_df)
length(id2_df_squish)


m_gp <- id2_df[id2_df[,1] %in% gp, ]

count_next <- sample(1:26, 3, replace = F)
data.frame(m_gp)[count_next,]

####


duplicated(unique2_id1)
...
uncounted <- id2[is.na(match_id)] # this finds where id2 did not match up to anything in id1
...
count_next <- sample(uncounted, 3, replace = F)
...
gp = c(426, 428, 443, 306, 307, 319)
 ...
 m = c(426,428,443,442,460,462)
 m_imgs_id2 <- id2[newsid2_anum %in% m ]
 
 
 wt_imgs_id2 <- id2[newsid2_anum %in% wt ]
