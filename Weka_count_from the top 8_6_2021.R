...
##***move the inv_managment functions up here eventually****
...
COUNTED <- paste(ORIGIN, "counted", sep = "")
for (i in 1:n){
  dir.create(paste(COUNTED,"/",nm,i,sep = ""))
}
...
inputdir_all1 <-"F:/Theo/iba_7_2020_autocount/Hina_IFNBKO_pair/working_images/new_val_train_etc/full_data_run_8bit/weka_output_thresh/class4/" 
...
id_for_dir <- "F:/Theo/iba_7_2020_autocount/Hina_IFNBKO_pair/working_images/new_val_train_etc/full_data_run_8bit/weka_output_thresh/"
...
#class_list is in the wrong order compared to class order, this will cut out the numbers at the end of my folders**regularize?
...
for (j in 1:length(class_list)){
  ...
  out_loc <- "F:/Theo/iba_7_2020_autocount/Hina_IFNBKO_pair/working_images/new_val_train_etc/full_data_run_8bit/weka_output_thresh_projected/"
...
counted_folder_dir <-"F:/Theo/iba_7_2020_autocount/Hina_IFNBKO_pair/working_images/new_val_train_etc/full_data_run_8bit/counted/"
setwd(counted_folder_dir)
class_list <- list.files()
...
f = 1 
 class_loc <- paste(counted_folder_dir,class_list[f],"/Results.csv",sep = "")
  ...
  #*#*#*#*# still need to remove the val images from the super DF
  ...
  #a vs auto
  t.test(superdf$count_Hina_hand[superdf$geno == "wt"] , superdf$count_new_auto[superdf$geno == "wt"])#S .041
  t.test(superdf$count_Hina_hand[superdf$geno == "gp"] , superdf$count_new_auto[superdf$geno == "gp"])#NS 0.1
  
  
  #b vs auto
  t.test(superdf$count_theo_hand[superdf$geno == "wt"] , superdf$count_new_auto[superdf$geno == "wt"])# NS 0.12
  t.test(superdf$count_theo_hand[superdf$geno == "gp"] , superdf$count_new_auto[superdf$geno == "gp"])# S 0.018
