...
astrocyte_genes=c("Aldh1l1", "Aqp4", "Sox9", "Gfap")# aldh1=astro promoter,sox9 = astro in adult brain, aqp=aquaporin; tried Glt1, but it didnt work, no matches, 
...
summary(model3_null)
#crplot(model3_null$, "coefficients", g="residuals") # not working
...
help(em_quantvg)
# model_subset <- em_quantvg(c(2,3,4,5), tnv=4, ng=2, int = T)# dont know what c(2,3,4,5) are, maybe columns in model_matrix
# there are 8 elements in model matrix, cant set int=F even though col 1 in model_matrix is just 1
# going to try the neuron, astro, microglia, NO OLIGO
...
summary(models[[2]][["ILMN_1212938"]])## testing aif

summary(models[[2]][["ILMN_1216463"]])## testing an NA in symbols


summary(models[[2]][["ILMN_1212938"]])## testing the top line in table, is all NA
#has no coef 2, 6, thats why table = all NA

# regressor_names <- as.character(1:9)
regressor_names <- as.character(1:9)# dont know why this is like this 
## added own names using model_matrix, didnt work at all
#regressor_names <- c("int","neuro_dif","astro_dif", "oligo_dif", "micro_dif","neuro_dif","astro_dif", "oligo_dif", "micro_dif" )
# 
# names(models)
# str(models)
# sum(is.na(models))
...
sum(is.na(coefficients))# =220255 nA, NAs may be ok as they correspond to model_subset designation of coefficients to use in model
...
#many NAs here, shouldnt there be NO NA?? **************************

order <- order(pvalues[filter,6]) # 6 = neuro diff
order_micro <- order(pvalues[filter,9]) # 9 = microglia diff
###NEED TO LOOK AT ORDER TO SEE WHEN RUN OUT OF ORDERING VARIABLE AND REVERT TO INDEX NUMBER
#neuronal expression levels, differences in neuronal expression in HD versus control, and their associated p-values


## coef 2 and 6 should be neuro ref and nero diff in model_matrix
table <- data.frame(coefficients[,c(2,6)], pvalues[,c(2,6)], symbols)[filter,][order,] # for neuro
table_micro <- data.frame(coefficients[,c(5,9)], pvalues[,c(5,9)], symbols)[filter,][order_micro,]
head(table)
head(table_micro)
...
models[[2]]["ILMN_1212938"]
symbols_aif1 <- getSYMBOL("ILMN_1212938", "illuminaMousev2.db")



#examining Anxa3, which may be sig diff bt genders?? and related to lcn2 for gender??
Anxa3 <- c("Anxa3")
...
#visualise and summarize illumina ref data selected cell type markers 
X11()
par(mfrow=c(1,4), cex=1.2)
...
#___________________ looking at AIf1 via gender
#ILMN_1212938

model_Aif1 <- lm(exprl["ILMN_1212938",] ~ neuron_reference + microglia_reference + astrocyte_reference + oligo_reference)


#visualise and summarize illumina ref data selected cell type markers on LCN2 in this case 
X11()
par(mfrow=c(1,4), cex=1.2)
crplot(model_Aif1, "neuron_reference", newplot=FALSE)
crplot(model_Aif1, "microglia_reference", newplot=FALSE)
crplot(model_Aif1, "astrocyte_reference", newplot=FALSE)
crplot(model_Aif1, "oligo_reference", newplot=FALSE)
summary(model_Aif1)

model2_Aif1_neuro <- lm(exprl["ILMN_1212938",] ~ neuron_reference +  neuron_difference)
#plot(model2_neuro)
crplot(model2_Aif1_neuro, "neuron_reference", g="neuron_difference")
summary(model2_Aif1_neuro)

model2_Aif1_micro <- lm(exprl["ILMN_1212938",] ~ microglia_reference + microglia_difference)
crplot(model2_Aif1_micro, "microglia_reference", g="microglia_difference")
summary(model2_Aif1_micro)

model2_Aif1_astro <- lm(exprl["ILMN_1212938",] ~ astrocyte_reference + astrocyte_difference)
crplot(model2_Aif1_astro, "astrocyte_reference", g="astrocyte_difference")
summary(model2_Aif1_astro)

model2_Aif1_oligo <- lm(exprl["ILMN_1212938",] ~ oligo_reference + oligo_difference)
crplot(model2_Aif1_oligo, "oligo_reference", g="oligo_difference")
summary(model2_Aif1_oligo)


########### Klk3, prostate cancer protease
#examining Anxa3, which may be sig diff bt genders?? and related to lcn2 for gender??
Klk6 <- c("Klk6")
Klk6_illumina_name <- mapIds(illuminaMousev2.db, keys=Klk6, column='PROBEID', keytype='SYMBOL',multiVals="list")
#gives 2, picked first one

model_Klk6 <- lm(exprl["ILMN_2775176",] ~ neuron_reference + microglia_reference + astrocyte_reference + oligo_reference)


#visualise and summarize illumina ref data selected cell type markers on LCN2 in this case 
X11()
par(mfrow=c(1,4), cex=1.2)
crplot(model_Klk6, "neuron_reference", newplot=FALSE)
crplot(model_Klk6, "microglia_reference", newplot=FALSE)
crplot(model_Klk6, "astrocyte_reference", newplot=FALSE)
crplot(model_Klk6, "oligo_reference", newplot=FALSE)
summary(model_Klk6)

#looking at gender for Klk6 
model2_Klk6_neuro <- lm(exprl["ILMN_2775176",] ~ neuron_reference +  neuron_difference)
#evaluating fem only and male only data intead of male
model2_Klk6_neuro2 <- lm(exprl["ILMN_2775176",] ~ neuron_reference +  neuron_difference2)

#plot(model2_neuro)
crplot(model2_Klk6_neuro, "neuron_reference", g="neuron_difference")
summary(model2_Klk6_neuro)

model2_Klk6_micro <- lm(exprl["ILMN_2775176",] ~ microglia_reference + microglia_difference)
crplot(model2_Klk6_micro, "microglia_reference", g="microglia_difference")
summary(model2_Klk6_micro)

model2_Klk6_astro <- lm(exprl["ILMN_2775176",] ~ astrocyte_reference + astrocyte_difference)
crplot(model2_Klk6_astro, "astrocyte_reference", g="astrocyte_difference")
summary(model2_Klk6_astro)

model2_Klk6_oligo <- lm(exprl["ILMN_2775176",] ~ oligo_reference + oligo_difference)
crplot(model2_Klk6_oligo, "oligo_reference", g="oligo_difference")
summary(model2_Klk6_oligo)

##trying second klk6 probeset###############################
model_Klk6 <- lm(exprl["ILMN_2955509",] ~ neuron_reference + microglia_reference + astrocyte_reference + oligo_reference)
...
#looking at gender for Klk6 
model2_Klk6_neuro <- lm(exprl["ILMN_2955509",] ~ neuron_reference +  neuron_difference)
#evaluating fem only and male only data intead of male
model2_Klk6_neuro2 <- lm(exprl["ILMN_2955509",] ~ neuron_reference +  neuron_difference2)
