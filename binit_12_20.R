...
binit <- function(input_data="~/bigdata/hic/bin/test_binit_datain", sbin=10000, sav.loc="./", ch.sizes="~/bigdata/hic/data/SRR2240738/mm10/mm10.chrom.sizes", chr_want= "chr1"){
  ...
  l = length(inputsplit)
  outfile_name = paste0(inputsplit[l],"_binit_out_test_12_20",chr_want) ###advisable to add date
  ######
  ...
  ##### removing any incomplete cases or levels that appear in read 2 but not read 1 (this means the parent with most chormosmomes needs to be "read 2" of "alternative genome")
  ...
  ##### removing all reads but those for a single chromosome #testsum <- sum(datain_test2$rd1_ch != datain_test2$rd2_ch)
