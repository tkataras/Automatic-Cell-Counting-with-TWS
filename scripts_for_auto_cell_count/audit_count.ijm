...
// Get the classifier
	...
	output = input + "/../../Audit_Counted/" + selectedClassifier + "/";
		// Audit Hand Counts
	dirVal = input + "/../../Audit_Hand_Counts/"+ selectedClassifier + "/";
		...
		function action(input, output, filename, inputTwo, filenameTwo) {    	
			...
			if (max == 0) {
				...
				if (endsWith(filenameTwo, ".roi")) {
					...
					print("this is roi name being used for hand count coords"  + Roi.getName); 
